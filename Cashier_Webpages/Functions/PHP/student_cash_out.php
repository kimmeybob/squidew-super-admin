...
if($query_response){
    ...
    if(bcsub($student_wallet_balance, $total_amount_deductable, 2) < 0){
        //echo "insufficient";
        echo $student_wallet_balance." | ";
        echo $total_amount_deductable." | ";
        echo bcsub($student_wallet_balance, $total_amount_deductable, 2);
    }else{
...
function createTransactionAndCashInTransaction($total_amount, $fee_amount, $cashin_student_id, $connection, $cashier_id){
    ...
    $create_cashier_transaction_query = "insert into transaction (transaction_amount, transaction_date, transaction_status, transaction_type) values (".$total_amount.",'".$current_date_and_time."','1','3');";
...
function DeductWalletBalance($student_wallet_id, $total_amount_to_be_deducted_no_fee, $student_wallet_balance, $connection){
    global $transaction_id;
    
    $new_wallet_balance = bcsub($student_wallet_balance, $total_amount_to_be_deducted_no_fee, 2);
